<?php
    ...
    if (isset($_SESSION['username'])&&($_SESSION['rank'])) {
        ...
        $queryVerificar = mysqli_query($db, "SELECT COUNT(*) as Existe FROM asignaciones WHERE Cod_Dep='$Cod_Dep' AND Cod_Cargo='$Cod_Cargo' AND Id_Empleado='$Id_Empleado'") or die(mysqli_error());
        ...
        if ($rowExiste['Existe']==1) {
            ...
            $queryBorrar = mysqli_query($db, "DELETE FROM asignaciones WHERE Cod_Dep='$Cod_Dep' AND Cod_Cargo='$Cod_Cargo' AND Id_Empleado='$Id_Empleado'") or die(mysqli_error());
