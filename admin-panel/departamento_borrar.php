<?php
    ...
    if (isset($_SESSION['username'])&&($_SESSION['rank'])) {
        ...
        $codigo = $_POST['codigo_depto'];

        // Variable que guarda el resultado de la consulta, para saber si existe
        $queryVerificar = mysqli_query($db, "SELECT COUNT(*) as Existe FROM departamentos WHERE Cod_Dep='$codigo'") or die(mysqli_error());
        ...
        if ($rowExiste['Existe']==1) {
            ...
            $queryBorrar = mysqli_query($db, "DELETE FROM departamentos WHERE Cod_Dep='$codigo'") or die(mysqli_error());
